from AyeshaBot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
from AyeshaBot import SUDO_USERS
...
async def saitama_is_admin(chat_id: int):
    status = False
    AyeshaBot = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if AyeshaBot.id == user.id:
            status = True
            break
    return status
